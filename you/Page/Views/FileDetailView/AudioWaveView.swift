...
struct AudioWaveView: View {
    @StateObject var conductor = AudioCompletionHandlerConductor()
    ...
    //需要异步调用生成结果波形图
    @StateObject var model = AudioViewModel(file: AudioViewModel.getFile(fileName: "test", fileExtension: "mp3"))
        ...
        VStack{
            ...
            HStack{
                ...
                Button {
                    ...
                    if conductor.player.currentTime < duration {
                        duration = conductor.player.currentTime
                    ...
                    conductor.player.seek(time: -(duration-0.01))
                ...
                Button {
                    ...
                    if conductor.player.status == .playing{
                        //在播放中，提供暂停操作
                        conductor.isPlaying = false
                    }else if conductor.player.status == .paused || conductor.player.status == .stopped{
                        //暂停后恢复
                        conductor.isPlaying = true
                    }
                    
                } label: {
                    Image(conductor.isPlaying ? "icon_pause_operate":"icon_play_operate")
                        .resizable()
                        .frame(width: 40,height:40)
                }.frame(maxWidth: .infinity)
