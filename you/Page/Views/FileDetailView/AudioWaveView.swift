...
struct AudioWaveView: View {
    ...
    var length :Double{
        ...
        return model.timeStamp/model.endTime
    }
    ...
    var body: some View {
        NavigationBody(title: "") {
            HStack{
                Image("icon_share").resizable().frame(width: 22,height: 22).onTapGesture {
                    print("分享")
                }
                
                Spacer().frame(width: 10)
                
                Image("icon_more").resizable().frame(width: 22,height: 22).onTapGesture {
                    print("更多")
                }
            }
        } content: {
                ...
                VStack{
                    VStack{
                        ZStack(alignment: .leading) {
                            ...
                            MinimapView(start: $start, length: length,indicatorSize: $indicatorSize)
                        }.frame(height: 100)
                        .padding()
                        ...
                        HStack{
                            ...
                            Button {
                                ...
                                if model.player.currentTime < duration {
                                    duration = model.player.currentTime
                                ...
                                model.player.seek(time: -(duration-0.01))
                            ...
                            Button {
                                ...
                                if model.player.status == .playing{
                                    //在播放中，提供暂停操作
                                    model.isPlaying = false
                                }else if model.player.status == .paused || model.player.status == .stopped{
                                    //暂停后恢复
                                    model.isPlaying = true
                                }
                                
                            } label: {
                                Image(model.isPlaying ? "icon_pause_operate":"icon_play_operate")
                                    .resizable()
                                    .frame(width: 40,height:40)
                            }.frame(maxWidth: .infinity)
                    ...
                    }.padding(.vertical,15).background(Color(hexString:"#F6F7F9"))
                    
                    Spacer().frame(height: 20)
                    //语音转文本的所有信息。使用@Binding绑定的数据源类型要一致，不能通过!修改数据源原有类型
                    //注意⚠️：内嵌TabView会导致顶部的标题栏出现问题，必须在TabView的View页面中设置取消顶部标题栏才能正常展示。参考首页的TabView的处理方式
                    AudioConvertTextView(fileOnCloudDetailData: $model.fileOnCloudDetailData)
