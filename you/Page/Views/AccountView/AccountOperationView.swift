...
//  you
//  账号操作【包括：注册、修改密码】关联页面关闭使用页面显示的状态
...
struct AccountOperationView: View {
    ...
    private var title : String{
        get{
            if type == 0{
                if model.isRegisterSuccess{
                    return "注册结果"
                }
                return "注册"
            }else{
                if model.isForgetSuccess{
                    return "密码修改结果"
                }
                return "忘记密码"
            }
    ...
    var body: some View {
        NavigationBody(title: title){
            //右侧的额外功能
        }content: {
            if model.isRegisterSuccess{
                ResultView(pageType: .Account(data: ResultData("注册结果",desc:"请到邮箱激活后登录",image: "completed",confirm:"去登录")))
            }else if model.isForgetSuccess{
                ResultView(pageType: .Account(data: ResultData("注册结果",desc:"请到邮箱修改密码后登录",image: "completed",confirm:"去登录")))
            }else{
