...
open class NetworkRequestHelper: NetworkBaseRequest {
    ...
    @discardableResult
    open class func get(path: String, parameters: Parameters? = nil, success: RequestSuccess?, failure: RequestFailure? = nil) -> DataRequest? {
        self.request(path: path, parameters: parameters, success: success, failure: failure)
    }
    
    @discardableResult
    open class func post(path: String, parameters: Parameters? = nil, success: RequestSuccess?, failure: RequestFailure? = nil) -> DataRequest? {
    ...
    @discardableResult
    open class func request(path: String, method: HTTPMethod = .get, parameters: Parameters? = nil, success: RequestSuccess?, failure: RequestFailure?) -> DataRequest? {
    ...
    ///   - success: call back when request success
    ///   - failure: call back when request failure
    open func process(response: AFDataResponse<Data?>, success: RequestSuccess?, failure: RequestFailure? = nil) {
    ...
    @discardableResult
    open class func download(path: String, method: HTTPMethod = .get, parameters: Parameters? = nil, success: RequestSuccess?, failure: RequestFailure? = nil, to destination: DownloadRequest.Destination? = nil) -> DownloadRequest? {
        // init with class object
        let req: NetworkRequestHelper = self.init(path: path)
        if req.prepareRequest(path: path, method: method, parameters: parameters) == false {
            return nil
    ...
    ///   - success: call back when request success
    ///   - failure: call back when request failure
    open func processDownload(response: AFDownloadResponse<URL?>, success: RequestSuccess?, failure: RequestFailure? = nil) {
        switch response.result {
        case let .success(data):
